...
`,
                                ...
                                try {
                                    await notifme_1.sendNotification(status === "down"
                                        ? `🟥 ${siteName} (${site.url}) is **down**: ${newIssue.data.html_url}`
                                        : `🟨 ${siteName} (${site.url}) is experiencing **degraded performance**: ${newIssue.data.html_url}`);
                                }
                                catch (error) {
                                    console.log(error);
                                }
                            ...
                            await octokit.issues.createComment({
                                ...
                                body: `**Resolved:** ${siteName} ${issues.data[0].title.includes("degraded")
                            ...
                            try {
                                await notifme_1.sendNotification(`🟩 ${siteName} (${site.url}) ${issues.data[0].title.includes("degraded")
                                    ? "performance has improved"
                                    : "is back up"}.`);
                            }
                            catch (error) {
